          ...
          return new Promise((res) => {
            ...
            logo.onload = () => {
              ...
              if (s.shape.type === "circle") {
                ctx.save();
                ctx.beginPath();
                ctx.arc(0, 0, Math.min(baseW, baseH) / 2, 0, Math.PI * 2);
                ctx.closePath();
                ctx.clip();
              }
              ...
              const maxW = baseW * (1 - paddingRatio);
              const maxH = baseH * (1 - paddingRatio);
              ...
              /* ===== BADGE ===== */
              const badgeRadius = Math.max(14, Math.min(baseW, baseH) * 0.12);
              const badgeY = -baseH / 2 - badgeRadius - 8;
              ...
              ctx.textAlign = "center";
              ctx.textBaseline = "middle";
              ctx.fillText(String(index + 1), 0, badgeY);
  ...
  const groupedImagesForPayload = useMemo(() => {
    return grouped.map((g) => {
      ...
      return {
        ...
        baseImage: preparedImages[g.imageIndex] || first.baseImage || null,
        maskImage: first.maskImage || null,

        // 🔥 ADICIONAR ISSO:
        removalAreas: removalAreas?.[g.imageIndex] || []
      };
    });
  }, [grouped, preparedImages, removalAreas]);
      ...
      <section className="email-validation email-validation--top">
        ...
        <div className="email-row">
          <input
            ...
            disabled={verified}
            className={!email.trim() && !verified ? "pulse-input" : ""}
          />

          <button
            className={`secondary pr-btn pr-outline validate-btn ${emailValid && !verified && !isSendingCode ? "pulse" : ""
              }`}
      ...
      <section className="project-top-row">
        ...
        <div className="project-top-col project-name-col">
          ...
          <input
            type="text"
            value={projectName}
            className={`input-project-name ${projectName.trim().length < 3 ? "pulse-input" : ""
              }`}
            onChange={(e) => {
